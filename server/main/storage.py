...
class DBStorageServer(DBStorage):
    ...
    def get_client_id(self, login: str):
        self._cursor.execute('SELECT `id` FROM `Clients`'
                             ' WHERE `login` == ?', (login,))
    ...
    def get_client_hash(self, login: str) -> str:
        self._cursor.execute('SELECT `info` FROM `Clients`'
                             ' WHERE `login` == ?', (login,))
        return self._cursor.fetchall()[0][0]
    ...
    def add_client(self, login: str, password_hash: str):
        ...
        if not password_hash:
            raise ValueError('паролm не может быть пустым')
        if self.check_client_exists(login) is True:
            raise RuntimeError(f'Gjkmpjdfntkm с этим логином уже существует:'
                               f' {login}')
        self._cursor.execute('INSERT INTO `Clients` '
                             'VALUES (NULL, ?, ?, NULL, NULL)',
                             (login, password_hash))
        self._conn.commit()

    def update_client(self, login: str, connection_time: float,
                      connection_ip: str):
    ...
    def check_client_in_contacts(self, owner_login: str,
                                 client_login: str) -> bool:
        owner_id = self.get_client_id(owner_login)
        client_id = self.get_client_id(client_login)
        self._cursor.execute('SELECT COUNT() FROM `ClientContacts` '
    ...
    def add_client_to_contacts(self, owner_login: str, client_login: str):
        owner_id = self.get_client_id(owner_login)
        client_id = self.get_client_id(client_login)
        self._cursor.execute('INSERT INTO `ClientContacts` VALUES (?, ?);',
                             (owner_id, client_id))
        self._conn.commit()
    ...
    def del_client_from_contacts(self, owner_login: str, client_login: str):
        owner_id = self.get_client_id(owner_login)
        client_id = self.get_client_id(client_login)
        self._cursor.execute('DELETE FROM `ClientContacts` '
                             'WHERE `owner_id` == ? AND `contact_id` == ?',
                             (owner_id, client_id))
        self._conn.commit()
    ...
    def get_client_contacts(self, client_login: str) -> list:
        ...
        where (`ClientContacts`.contact_id == `Clients`.id
        and `ClientContacts`.owner_id == ?);'''
...
class DBStorageClient(DBStorage):
    ...
    def add_contact(self, login: str):
        self._cursor.execute('INSERT INTO `Contacts` '
                             'VALUES(NULL, ?)', (login,))
        self._conn.commit()

    def get_contact_id(self, login: str):
        self._cursor.execute('SELECT `id` FROM `Contacts` '
                             'WHERE `login` == ?', (login,))
        try:
            return self._cursor.fetchall()[0][0]
        except IndexError:
    ...
    def add_message(self, login: str, text: str, incoming: bool = False):
        ...
        self._cursor.execute('INSERT INTO `Messages` VALUES (NULL, ?, ?, ?)',
                             (contact_id, int(incoming), text))
        self._conn.commit()
    ...
    def delete_contact(self, login: str):
        self._cursor.execute('DELETE FROM `Contacts` '
                             'WHERE `login` == ?', (login,))
        self._conn.commit()
