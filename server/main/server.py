...
from server.main.jim import request_from_bytes, JimResponse, \
    auth_server_message
...
def parse_commandline_args(cmd_args):
    ...
    parser.add_argument('-a', dest='listen_address',
                        type=str,
                        default='')
    parser.add_argument('-p', dest='listen_port',
                        type=int,
    ...
    def __init__(cls, clsname, bases, clsdict):
        ...
        for key, value in clsdict.items():
            ...
            if '.connect(' in source:
                raise RuntimeError('Сервер не использует '
                                   'соединение для сокетов')
    ...
    def mainloop(self):
        ...
        while True:
            ...
            try:
                ...
                try:
                    readable, writable, erroneous = select.select(clients,
                                                                  clients,
                                                                  clients,
                                                                  0)
                except IndexError:
                ...
                for client_socket in readable:
                    try:
                        ...
                        request = \
                            request_from_bytes(
                                client_socket.recv(
                                    helpers.TCP_MSG_BUFFER_SIZE))
                        self.__print_queue.put(f'Запрос:\n{request}')
                        responses = []
                        if request.action == 'presence':
                            client_login = \
                                request.datadict['user']['account_name']
                            resp = JimResponse()
                            if not self.storage.check_client_exists(
                                    client_login):
                                resp.response = 400
                                resp.set_field('error',
                                               f'No such client: '
                                               f'{client_login}')
                                ...
                                self.storage.update_client(client_login,
                                                           client_time,
                                                           client_ip)
                                resp.response = 200
                            else:
                            ...
                            client_login \
                                = request.datadict['user']['account_name']
                            client_hash \
                                = self.storage.get_client_hash(client_login)
                            auth_token = auth_tokens[client_socket]
                            del auth_tokens[client_socket]
                            expected_digest =\
                                security.create_auth_digest(client_hash,
                                                            auth_token)
                            client_digest = \
                                request.datadict['user']['password']
                            resp = JimResponse()
                            if not security.check_auth_digest_equal(
                                    expected_digest,
                                    client_digest):
                                resp.response = 402
                            ...
                            if not self.storage.check_client_exists(
                                ...
                                resp.response = 400
                                resp.set_field('error',
                                               f'Такого клиента не существует:'
                                               f'{contact_login}')
                            elif self.storage.check_client_in_contacts(
                                    client_login,
                                    contact_login):
                                resp.response = 400
                                resp.set_field('error',
                                               f'Клиент уже '
                                               f'находится в контактах:'
                                               f' {contact_login}')
                            else:
                                self.storage.add_client_to_contacts(
                                    client_login,
                                    contact_login)
                                resp.response = 200
                            responses.append(resp)
                            ...
                            if not self.storage.check_client_exists(
                                ...
                                resp.set_field('error',
                                               ...
                                               f' не существует: '
                                               f'{contact_login}')
                            elif not self.storage.check_client_in_contacts(
                                    client_login,
                                    contact_login):
                                resp.response = 400
                                resp.set_field('error',
                                ...
                                self.storage.del_client_from_contacts(
                                    client_login,
                                    contact_login)
                                resp.response = 200
                            responses.append(resp)
                            ...
                            client_login = logins[client_socket]
                            client_contacts = \
                                self.storage.get_client_contacts(
                                    client_login)
                            quantity_resp = JimResponse()
                            quantity_resp.response = 202
                            quantity_resp.set_field('quantity',
                                                    len(client_contacts))
                            responses.append(quantity_resp)
                            for contact in client_contacts:
                                contact_resp = JimResponse()
                                contact_resp.set_field('action',
                                                       'contact_list')
                                contact_resp.set_field('user_id',
                                                       contact)
                            ...
                            for key, val in logins.items():
                                ...
                                resp.response = 400
                                resp.set_field('error',
                                               f'Такого клиента нет в сети: '
                                               f'{target_client_login}')
                            responses.append(resp)
                        else:
                            raise RuntimeError
                        ...
                        self.__print_queue.put(f'Клиент отключен: '
