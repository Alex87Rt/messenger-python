...
from client.main.jim import request_from_bytes
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def disconnect_click(self):
        if not self.client:
            self.print_info('Нет связи')
            return
        self.print_info('Отключение')
        self.clear_state()
    ...
    def send_message_click(self):
        try:
            ...
            if not selected_contacts:
                self.print_info('Контакт не выбран')
                return
            login = selected_contacts[0].text()
            text = self.ui.textEdit_input.toPlainText()
            if not text:
                self.print_info('Текст сообщения не может быть пустым')
                return
