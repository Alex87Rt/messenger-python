...
class DBStorageServer(DBStorage):
    ...
    def add_client(self, login: str, password_hash: str):
        ...
        if self.check_client_exists(login) is True:
            raise RuntimeError(f'client with this login '
                               f'already exists: {login}')
...
class DBStorageClient(DBStorage):
    ...
    def add_contact(self, login: str):
        self._cursor.execute('INSERT INTO `Contacts` VALUES(NULL, ?)',
                             (login,))
        self._conn.commit()

    def get_contact_id(self, login: str):
        self._cursor.execute('SELECT `id` FROM `Contacts` WHERE `login` == ?',
                             (login,))
        try:
            return self._cursor.fetchall()[0][0]
        except IndexError:
    ...
    def delete_contact(self, login: str):
        self._cursor.execute('DELETE FROM `Contacts` WHERE `login` == ?',
                             (login,))
        self._conn.commit()
